...
describe('ActionsOnGoogleTestManager', function () {
  ...
  it('should set default locale and surface', async function () {
    ...
    assert.deepOwnInclude(test.getTestInteractionMergedDefaults(), {
  ...
  it('should get and assert speech from latest response', async function () {
    ...
    expect(() =>
      test.assertSpeech(expectedSpeech, {
        isExact: true,
      })
    ).not.to.throw();
    expect(() => test.assertSpeech('Welcome')).not.to.throw();
    expect(() =>
      test.assertSpeech(['Welcome to .* about Google!'], {
        isRegexp: true,
      })
    ).not.to.throw();
    expect(() =>
      test.assertSpeech(['no match', 'Welcome to Facts about Google!'])
    ).not.to.throw();
    expect(() => test.assertSpeech('bad')).to.throw();
    expect(() => test.assertSpeech(['bad', 'bad2'])).to.throw();
    expect(() =>
      test.assertSpeech(['bad .* about Google!', 'bad'], {
        isRegexp: true,
      })
    ).to.throw();
    expect(() =>
      test.assertSpeech(['Welcome to Facts about Google!'], {
        isExact: true,
      })
    ).to.throw();
  });
  ...
  it('should get and assert text from latest response', async function () {
    ...
    expect(() =>
      test.assertText(['Welcome to .* about Google!'], {
        isRegexp: true,
      })
    ).not.to.throw();
    expect(() =>
      test.assertText(['no match', 'Welcome to Facts about Google!'])
    ).not.to.throw();
    expect(() => test.assertText('bad')).to.throw();
    expect(() => test.assertText(['bad', 'bad2'])).to.throw();
    expect(() =>
      test.assertText(['bad .* about Google!', 'bad'], {
        isRegexp: true,
      })
    ).to.throw();
    expect(() =>
      test.assertText(['Welcome to Facts about Google!'], {
        isExact: true,
      })
    ).to.throw();
  });
  ...
  it('should get and assert intent parameter from latest response', async function () {
    ...
    expect(() =>
      test.assertIntentParameter('intentParamString', 'value')
    ).not.to.throw();
    expect(() =>
      test.assertIntentParameter('intentParamInt', 5)
    ).not.to.throw();
    expect(() =>
      test.assertIntentParameter('BadIntentParamName', 'blabla')
    ).to.throw();
    expect(() =>
      test.assertIntentParameter('intentParamString', 'blabla')
    ).to.throw();
    expect(() => test.assertIntentParameter('intentParamInt', 101)).to.throw();
    expect(() =>
      test.assertIntentParameter('intentParamInt', 'blabla')
    ).to.throw();
  });
  ...
  it('should get and assert session parameters from latest response', async function () {
    ...
    expect(() =>
      test.assertSessionParam('categoryType', 'cats')
    ).not.to.throw();
    expect(() => test.assertSessionParam('factsNumber', 1)).not.to.throw();
    expect(() =>
      test.assertSessionParam('currentFactsData', {
  ...
  it('should get and assert user parameters from latest response', async function () {
    ...
    expect(() =>
      test.assertUserParam('userCategoryType', 'cats')
    ).not.to.throw();
  ...
  it('should get home parameters from latest response', async () => {
    ...
    expect(() =>
      test.assertHomeParam('homeStorageParam', 'home param value')
    ).not.to.throw();
  ...
  it('should get and assert suggestions from latest response', async () => {
    ...
    expect(() =>
      test.assertSuggestions(EXAMPLE_SUGGESTIONS, true)
    ).not.to.throw();
    expect(() => test.assertSuggestions(EXAMPLE_SUGGESTIONS)).not.to.throw();
    expect(() => test.assertSuggestions([{title: 'Headquarters'}])).to.throw();
  });
  ...
  it('should get and assert list from latest response', async () => {
    ...
    expect(() =>
      test.assertList({title: EXAMPLE_LIST!.title}, true)
    ).to.throw();
  ...
  it('should get and assert collection from latest response', async () => {
    ...
    expect(() =>
      test.assertCollection(EXAMPLE_COLLECTION, true)
    ).not.to.throw();
    expect(() =>
  ...
  it('should get and assert table from latest response', async () => {
    ...
    expect(() =>
      test.assertTable({title: EXAMPLE_TABLE!.title}, true)
    ).to.throw();
  ...
  it('should get and assert media from latest response', async () => {
    ...
    expect(() =>
      test.assertMedia({mediaType: EXAMPLE_MEDIA!.mediaType}, true)
    ).to.throw();
  ...
  it('should get and assert canvas data from latest response', async () => {
    ...
    expect(() =>
      test.assertCanvasData(EXAMPLE_CANVAS!.data!, true)
    ).not.to.throw();
    expect(() =>
      test.assertCanvasData([{elem1Key1: 'value'}, {}])
    ).not.to.throw();
    expect(() =>
      test.assertCanvasData([{elem1Key1: 'value'}, {elem2Key1: 'value2'}])
    ).not.to.throw();

    expect(() =>
      test.assertCanvasData([{elem1Key1: 'value'}], true)
    ).to.throw();
    expect(() =>
      test.assertCanvasData([{elem1Key1: 'bad value'}, {elem2Key1: 'value2'}])
    ).to.throw();
  ...
  it('should detect when a conversation has ended', async () => {
    initLatestResponse();
    test.latestResponse!.diagnostics!.actionsBuilderEvents!.slice(
      -1
    )[0]!.endConversation = true;
